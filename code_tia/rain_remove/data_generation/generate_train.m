...
%%% generate 2,000 h5 files, each file contains 500 image patches, total 1,000,000 patches
num_h5_files = 2000;  
num_patches = 500; 
%%% 
...
for j = 1:num_h5_files   
  ...
  for i = 1 : num_patches
    ...
    im_input = imread(strcat('./image/input/', listing(r_idx).name));  % input: rainy image
    im_input = double(im_input)./255.0;
        
    im_label = imread(strcat('./image/label/', listing(r_idx).name));  % label: clean image
